...
_M.local_screen = screen[mouse.screen]
...
function _M.preview()
	...
	local W = _M.local_screen.geometry.width  - 2 * _M.preview_wbox.border_width
	local w = W / n -- widget width
	local h = w * 0.75  -- widget height
	local textboxHeight = w * 0.125

	local x = _M.local_screen.geometry.x --+ _M.preview_wbox.border_width
	local y = _M.local_screen.geometry.y + (_M.local_screen.geometry.height - h - textboxHeight) / 2
